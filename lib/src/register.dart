...
class _MyRegisterWidget extends State<RegisterWidget>
  ...
  void _loadSettings() async {
    ...
    this.setState(() {
      _wsUriController.text =
          _preferences.getString('ws_uri') ?? 'wss://sip.pressone.co:443/wss';
      _sipUriController.text =
          _preferences.getString('sip_uri') ?? '1001@chevron';
      _displayNameController.text =
          _preferences.getString('display_name') ?? 'Flutter SIP UA';
      _passwordController.text = _preferences.getString('password') ?? 'mpirk4gw';
      _authorizationUserController.text =
          _preferences.getString('auth_user') ?? '1001';
    });
  }
  ...
  void registrationStateChanged(RegistrationState state) {
    ...
    logger.i(
        'Line user Registration State: ${EnumHelper.getName(_registerState.state)}. Caused by ${state.cause}');
  }
  ...
  void _handleSave(BuildContext context) {
    ...
    settings.iceServers = [{
      ...
      "username":"3954a9ba53ca219b4e08e17e4436b936c239c07e01b8efeb43a673043bef1ecf",
  ...
  void callStateChanged(Call call, CallState state) {
    //NO OP
    logger.i('Connection State: ${EnumHelper.getName(_registerState.state)}. Caused by ${state.cause}');
  }

  @override
  void transportStateChanged(TransportState state) {
    logger.i(
        'Line user Transport State: ${EnumHelper.getName(_registerState.state)}. Caused by ${state.cause}');
  }

  @override
  void onNewMessage(SIPMessageRequest msg) {
    // NO OP
    logger.i('New Message Received on user: ${msg.message}');
  }
}
